...
function getOriginalCodeForNode(opts, node) {
  ...
  const loc = node.loc;
  if (!loc) {
    return (0, import_generator.default)(node, {
...
var options = {
  ...
  markoAttrParen: {
    ...
    default: (() => {
      try {
        const rootRequire = (0, import_module.createRequire)(defaultFilePath);
        let compilerRequire;
        try {
...
var parsers = {
  marko: {
    ...
    parse(text, _parsers, opts) {
      ...
      const { ast } = (0, import_compiler.compileSync)(`${text}
`, filepath, {
        ast: true,
        code: false,
        optimize: false,
      ...
      if (opts.markoSyntax === "auto") {
        ...
        for (const childNode of ast.program.body) {
          if (import_compiler.types.isMarkoTag(childNode)) {
            if (import_compiler.types.isStringLiteral(childNode.name) && childNode.name.value === "style" && styleReg.exec(childNode.rawValue || "style")[0].endsWith("{")) {
              continue;
            }
    ...
    print(path, opts, print) {
      ...
      switch (node.type) {
        ...
        case "MarkoTag": {
          ...
          const literalTagName = import_compiler.types.isStringLiteral(node.name) ? node.name.value : "";
        ...
        case "MarkoAttribute": {
          ...
          if (node.default || !import_compiler.types.isBooleanLiteral(value, { value: true })) {
